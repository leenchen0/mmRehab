...
def moving_average(data, window_size, axis=0):
    cumsum = np.cumsum(data, axis=axis, dtype=float)
    cumsum[window_size:] = cumsum[window_size:] - cumsum[:-window_size]
    return cumsum / window_size

# def moving_average(data, window_size, axis=0):

#     weights = np.ones(window_size) / window_size
#     result = np.apply_along_axis(lambda m: np.convolve(m, weights, mode='same'), axis=axis, arr=data)
#     return result
...
def is_point_in_list(point, point_list):
    ...
    for p in point_list:
        # print(f"point: {point} p: {p} {np.array_equal(point, p)}") 
        if np.allclose(point, p):
            return True
    return False

def count_intersections(line_start, line_end, mesh_vertices, mesh_faces):
    count = 0
    ...
    for face in mesh_faces:
        ...
        intersection_point = ray_triangle_intersection(line_start, ray_direction, triangle)
        if intersection_point is not None and not is_point_in_list(intersection_point, intersection_points):
...
def prepare_display(data, data_gt=None, mesh=None, origin=[0, 0, 0], show_radar=False, voxel=False):
    ...
    if mesh is not None:
        ...
        o3d_mesh.triangles = o3d.utility.Vector3iVector(mesh['vertices'])
